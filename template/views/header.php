<div class="d-flex header fixed-top">

    <nav class="navbar_header ">
        <div class="header_logo"><img src="https://cdn.worldvectorlogo.com/logos/credit-immobilier-de-france.svg" alt=""></div>
        <ul class="head_nav d-flex desktop">
            <li class="header_item"><a href="?path=home">Home</a></li>
            <li class="header_item"><a href="?path=shop">Boutique</a></li>
            <li class="header_item"><a href="?path=account">Compte</a></li>
            <li class="header_item"><a href="?path=admin">Admin</a></li>
            <li class="header_item"><a href="?path=account"><i class="fas fa-user"></i></a></li>
            <li class="header_item"><a href="?path=cart"><i class="fas fa-cart-plus"></i></a></li>
        </ul>
         <!-- start display mobile -->
        <div class="mobile">
            <ul class="head_nav">
                <li class="header_item"><a href="?path=home"><i class="fas fa-home"></i></a></li>
                <li class="header_item"><a href="?path=account"><i class="fas fa-user"></i></a></li>
                <li class="header_item"><a href="?path=cart"><i class="fas fa-cart-plus"></i></a></li>

                <li id="menu"><a href=""><img src="https://img.icons8.com/material/24/000000/squared-menu--v1.png"/></a></li>
            </ul>
        </div>
    </nav>
    <div class="side_bar">
        <li class="header_item d-flex justify-content-between"><a href="?path=home">Home</a> <a href=""><img id="close" src="https://img.icons8.com/material/24/000000/multiply--v1.png"/></a></li>
        <li class="header_item"><a href="?path=shop">Boutique</a></li>
        <li class="header_item"><a href="?path=account">Compte</a></li>
        <li class="header_item"><a href="?path=cart">Panier</a></li>
    </div> 
    <!-- end display mobile-->   
</div>
